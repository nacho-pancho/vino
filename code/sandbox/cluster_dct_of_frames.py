...
while (cap.isOpened()):
    ...
    if not ret:
        break
    frame = cv2.resize(frame, (462, 260), fx = 0, fy = 0, interpolation = cv2.INTER_CUBIC)
    ...
    for j in range(len(patches)):
        dcts[j,:] = fft.dct(fft.dct(patches[j,:].reshape((w,w)),axis=1,norm="ortho"),axis=0,norm="ortho").reshape((1,m))
        dcts[j,0] = 0 # kill DC
